...
class GoalFunction(ReprMixin, ABC):
    ...
    def init_attack_example(self, attacked_text):
        ...
        self.num_queries = 0
        result, _, result_df = self.get_result(attacked_text, check_skip=True)
    ...
    def get_result(self, attacked_text, **kwargs):
        ...
        ``AttackedText`` object."""
        results, search_over, result_df = self.get_results([attacked_text], **kwargs)
        result = results[0] if len(results) else None
        return result, search_over, result_df
    ...
    def get_results(self, attacked_text_list, check_skip=False):
        ...
        model_outputs, result_df = self._call_model(attacked_text_list)
        ...
        return results, self.num_queries == self.query_budget, result_df
...
class AdvPromptGoalFunction(GoalFunction):
    ...
    def _call_model(self, attacked_text_list):
        ...
        for attacked_prompt in attacked_text_list:
            ...
            acc, result_df = self.inference.predict(prompt)
            if self.verbose:
    ...
    def _call_model(self, text_list):
        ...
        for prompt in text_list:
            ...
            if self.verbose:
                self.logger.info("Current acc: {:.2f}".format(acc*100))
            acc_list.append(acc)
        return acc_list, final_result_df
